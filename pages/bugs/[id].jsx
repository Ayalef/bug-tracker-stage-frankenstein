...
export default function Bugspage() {
  ...
  async function handleDeleteBug(id) {
    ...
    const apiUrlEndpoint = `http://localhost:3000/api/Deletebugs/`;
    const postData = {
      method: "POST",
        ...
        {dataResponse.map((products) => {
          return (
            <div key={products.id_bug} >
              ...
              <div className="flex flex-row bg-[#7D6E83]   text-white box-content h-3/4 w-96 border-4 border-solid  rounded-lg space-y-4  mr-10 ml-72 mt-20 " >
                  
                 
                <label className='flex flex-col ml-6 h-full font-mono text-black ' >
                 
                  <button  className='bg-[#4D455D]  text-white font-bold  rounded w-12 ml-72 mt-2 'onClick={() => router.push("/Editbugs/" + products.id_bug )} >Edit</button>
                  
                  <button  className='bg-[#4D455D] text-white font-bold  rounded w-12 ml-72 mt-1 ' onClick={() => handleDeleteBug(products.id_bug)}>✕</button>
               
                 
                
                  <h1 className='text-white mt-1 text-lg  ' >Bug name:</h1>
                  <label className='bg-white rounded text-lg text-gray-700 text-center w-64' >{products.nombre_bug}</label>
